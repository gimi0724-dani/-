...
**arbitrary value (`mb-[8px]`, `p-[5px]` 등)는 이 프로젝트에서 컴파일되지 않음.** 커스텀 픽셀 값은 반드시 인라인 `style={{ marginBottom: 8 }}` 형태로 작성할 것. `mb-3`, `px-4` 등 표준 클래스는 정상 동작.
...
### Phase 3 - 핵심 UI ✅
- [x] `App.tsx` — 전체 레이아웃 (Header + ChatFeed + InputBar)
- [x] `components/layout/InputBar.tsx` — 하단 고정 입력창 + 전송 로직
- [x] `components/chat/TransactionBubble.tsx` — 개별 거래 말풍선 (상호명+금액 한 줄 배치)
- [x] `components/chat/ChatFeed.tsx` — 스크롤 피드 + DateDivider
- [x] `components/layout/Header.tsx` — 월 요약 + 월 이동 버튼
- [x] `components/chat/ParsePreview.tsx` — 타이핑 중 실시간 파싱 미리보기

### Phase 4 - 학습 기능 + 수정 ✅
- [x] `components/modals/EditModal.tsx` — 카테고리/금액/메모 수정 UI
- [x] `src/hooks/useParser.ts` — merchantMap 조회 + 파싱 통합
- [x] 수정 시 merchantMap 자동 업데이트 연결

### Phase 5 - 리포트 및 차트 ✅
- [x] `src/hooks/useSummary.ts` — 월별 집계 계산
- [x] `components/modals/ChartModal.tsx` — Recharts 파이차트 + 바차트 + 카테고리 목록
- [x] `components/modals/ReportModal.tsx` — 월 요약, 카테고리 TOP5 프로그레스 바, 상호명 TOP3
- [x] `components/modals/SettingsModal.tsx` — 앱 정보 + 전체 데이터 초기화 (2단계 확인)
- [x] `db/transactions.ts` — `clearTransactions()` 추가
- [x] App.tsx 차트/리포트/설정 버튼 연결
